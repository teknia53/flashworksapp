...
function configToPrefs(cfg: CodesConfig): SessionPrefs {
  ...
  return {
    ...
    lowChpt: Math.max(4, Math.min(36, cfg.dbLowChpt)),
    highChpt: Math.max(4, Math.min(36, cfg.dbHighChpt)),
    searchFrequency: cfg.dbSearchFrequency === 1,
    lowFreq: Math.max(1, Math.min(20000, cfg.dbLowFreq)),
    highFreq: Math.max(1, Math.min(20000, cfg.dbHighFreq)),
